...
| rmuc2026_field | RMUC2026 imported STEP geometry | RMUC2026 multilayer PLY | The shared Gazebo field used to compare MeshNav and JIE, including ramps and a tunnel. |
...
### RMUC2026 holonomic demo base

For `world_name:=rmuc2026_field`, the simulated base uses a slope-aware planar
velocity system. It controls body-frame `linear.x`, `linear.y`, and `angular.z`
while leaving vertical, roll, and pitch dynamics free, allowing the robot to
conform to mesh slopes. The launcher also sets `mesh_controller.holonomic: true`
for the RMUC2026 map. Other tutorial worlds retain the original wheel-driven
DiffDrive plugin and non-holonomic controller profile. In the RMUC profile, an
independent Gazebo odometry publisher supplies planar pose, `vx`, `vy`, and yaw
rate.
