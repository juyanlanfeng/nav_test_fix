...
JIE can now model horizontal footprint and upward clearance independently:
...
The PCD GUI requires the system Open3D package:

```bash
sudo apt install python3-open3d
```

For a headless launch, add `start_import_gui:=false` and publish the absolute PCD path on `/pcd_file_cmd` separately.
