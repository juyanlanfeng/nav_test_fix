...
public:
  ...
  : Node("pcd_to_octomap")
  {
    ...
    // The publisher is transient-local, so late subscribers receive the last
    // map without periodic serialization.  A positive value restores the old
    // timer behavior for deployments which explicitly require it.
    ...
    const double republish_period_s = get_parameter("republish_period_s").as_double();
    if (republish_period_s > 0.0) {
      timer_ = create_wall_timer(
        std::chrono::duration<double>(republish_period_s),
        std::bind(&PcdToOctomapNode::publishMap, this));
      RCLCPP_WARN(
        get_logger(),
        "Periodic OctoMap republishing enabled at %.3f s; transient-local delivery "
