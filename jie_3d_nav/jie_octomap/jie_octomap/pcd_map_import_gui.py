...
class PcdMapImportNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("rmuc2026_profile", False)
        self.declare_parameter("initial_resolution", 0.5)
        self.declare_parameter("initial_preprocess_downsample_m", 0.1)
        qos = QoSProfile(
...
class PcdMapImportWindow(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.downsample_spin.setRange(0.0, 0.0 if self._rmuc2026_profile else 2.0)
        self.downsample_spin.setSingleStep(0.05)
        self.downsample_spin.setValue(
            0.0
            if self._rmuc2026_profile
            else float(
                self._ros_node.get_parameter(
                    "initial_preprocess_downsample_m"
                ).value
            )
        )
