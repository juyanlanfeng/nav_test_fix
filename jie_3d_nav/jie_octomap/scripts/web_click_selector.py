...
class WebClickSelectorNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("robot_radius_xy", -1.0)
        ...
        self.create_subscription(
            Marker, preblocked_marker_topic, self._on_preblocked, transient_qos
        )
        ...
        self.get_logger().info(
            "web_click_selector started. "
            f"raw_click_topic={raw_click_topic} "
            f"occupied_marker_topic={occupied_marker_topic}"
        )
    ...
    def _on_raw_click(self, msg: PointStamped) -> None:
        ...
        if snapped is None:
            ...
            self.get_logger().warn(
                f"Raw click [{msg.point.x:.3f}, {msg.point.y:.3f}, "
                f"{msg.point.z:.3f}] could not be snapped to a traversable cell."
            )
            return
    ...
    def _ground_support_depth(self, idx: GridIndex) -> Optional[int]:
    ...
    def _is_traversable(self, idx: GridIndex) -> bool:
        ...
        support_depth = 0
        if require_ground:
            resolved_support_depth = self._ground_support_depth(idx)
            if resolved_support_depth is None:
                return False
            support_depth = resolved_support_depth

        # Match jie_path_node's vertical discontinuity guard, including maps
        # whose signed voxel indices extend below z=0.
        minimum_z = self.min_idx[2] if self.min_idx is not None else idx[2]
        for below_z in range(idx[2] - 1, minimum_z - 1, -1):
            below = (idx[0], idx[1], below_z)
            if self._is_occupied(below):
                break
            if below in self.preblocked_cells:
                return False

        legacy_radius = max(0.0, float(self.get_parameter("robot_radius").value))
        configured_radius_xy = float(self.get_parameter("robot_radius_xy").value)
        configured_height = float(self.get_parameter("robot_height").value)
        use_legacy_hemisphere = configured_radius_xy <= 0.0 and configured_height <= 0.0
        radius_xy = configured_radius_xy if configured_radius_xy > 0.0 else legacy_radius
        robot_height = configured_height if configured_height > 0.0 else legacy_radius
        n_xy = max(0, math.ceil(radius_xy / self.resolution))
        if use_legacy_hemisphere:
            min_dz = 0
            max_dz = max(0, math.ceil(robot_height / self.resolution))
        else:
            min_dz = 1 - support_depth if support_depth > 0 else 0
            max_dz = math.floor(
                (robot_height + 1.0e-12) / self.resolution
            ) - support_depth

        for dx in range(-n_xy, n_xy + 1):
            for dy in range(-n_xy, n_xy + 1):
                horizontal_sq = (dx * self.resolution) ** 2 + (dy * self.resolution) ** 2
                if horizontal_sq > radius_xy * radius_xy + 1.0e-12:
                    continue
                for dz in range(min_dz, max_dz + 1):
                    vertical = dz * self.resolution
                    if use_legacy_hemisphere and (
                        horizontal_sq + vertical * vertical
                        > legacy_radius * legacy_radius + 1.0e-12
                    ):
                        continue
                    nearby = (idx[0] + dx, idx[1] + dy, idx[2] + dz)
                    if nearby in self.preblocked_cells or self._is_occupied(nearby):
                        return False
        return True
    ...
    def _make_arrow(
        self, marker_id: int, p: PointStamped, rgb: Tuple[float, float, float]
    ) -> Marker:
        marker = Marker()
