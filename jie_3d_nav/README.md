...
JIE 的碰撞体现在可分别描述水平占地和向上净空：

- `robot_radius_xy` 是 XY 平面圆形包络半径；
- `robot_height` 是从实际支撑/地面到车顶的物理高度，不是从候选自由体素中心再向上增加一段高度；
- 当前 RMUC Ceres 仿真代理的推荐值是 `robot_radius_xy:=0.28`、`robot_height:=0.225`（不是未经验证的真机实测极限）。调参后的外轮半宽约为 0.245 m，所以 0.28 m 留有水平余量；但它仍是无航向圆柱近似，不等于任意姿态下矩形车体的精确外接包络；
- 旧配置仍可只设置 `robot_radius`。当两个新参数均未设置（或不大于 0）时，规划器保持原来的半球检测；只设置一个新参数时，另一个轴回退到 `robot_radius`。

通用导入继续保持原来的 `0.5 m` 默认值。`rmuc2026_profile:=true` 是显式的场地图 profile：它选择 `resolution=0.05 m`、0 预降采样、0.28 m 水平包络和 0.225 m 物理高度，并把 GUI 的推荐分辨率上限锁为 0.05 m。RMUC 约 0.246 m 净空不能用 0.1 m OctoMap可靠表示；0.1 m 会按预期拒绝真隧道。
...
无图形桌面时可加 `start_import_gui:=false`，再手动向 `/pcd_file_cmd` 发布 PCD 绝对路径。
