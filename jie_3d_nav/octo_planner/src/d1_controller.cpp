...
public:
    ...
    goal_reached_(false)
  {
    ...
    const auto debug_period = std::chrono::duration<double>(
      1.0 / std::max(
        1.0,
        debug_view_frequency));
...
private:
  ...
  void onStartNavigation(const std_msgs::msg::Bool::SharedPtr msg)
  {
    ...
    if (pending_plan_.empty()) {
      RCLCPP_WARN(
        get_logger(), "Start navigation requested, but no pending planned_path is available.");
      return;
    }
  ...
  void onControlTimerImpl()
  {
    ...
    cmd_vel.angular.z = clamp(
      ...
      get_parameter("max_angular_speed").as_double());
    cmd_vel.linear.x =
      applyDeadband(cmd_vel.linear.x, get_parameter("linear_deadband").as_double());
    cmd_vel.linear.y =
      applyDeadband(cmd_vel.linear.y, get_parameter("lateral_deadband").as_double());
    cmd_vel.angular.z = applyDeadband(
      cmd_vel.angular.z, get_parameter(
        "angular_deadband").as_double());
  ...
  bool selectTrackingTarget(TrackingTarget & target)
  {
    ...
    if (target_dist < reached_tolerance &&
  ...
  void renderTrackingDebugViewImpl()
  {
    ...
    cv::line(
      image, cv::Point(center.x, 0), cv::Point(center.x, image_size), cv::Scalar(
        48, 64,
        70), 1);
    cv::line(
      image, cv::Point(0, center.y), cv::Point(image_size, center.y), cv::Scalar(
        48, 64,
        70), 1);
    cv::arrowedLine(
    ...
    for (std::size_t i = 1; i < projected_points.size(); ++i) {
      cv::line(
        image, projected_points[i - 1], projected_points[i], cv::Scalar(
          120, 120,
          120), 1,
        cv::LINE_AA);
    }
    ...
    double & yaw_error) const
  {
    ...
    const cv::Point arrow_end(
      static_cast<int>(std::round(
        final_point.x - std::sin(goal_yaw_in_robot_frame) *
        arrow_length_px)),
      static_cast<int>(std::round(
        final_point.y - std::cos(goal_yaw_in_robot_frame) *
        arrow_length_px)));
    ...
    const geometry_msgs::msg::PoseStamped & final_pose_map)
  {
    ...
    cmd_vel.linear.x =
      clamp(final_pose_base.pose.position.x * linear_gain, -max_linear, max_linear);
    if (enable_lateral_motion) {
      cmd_vel.linear.y =
    ...
    const bool final_yaw_reached =
      !align_final_yaw ||
      std::abs(final_yaw_error) < get_parameter("goal_yaw_tolerance").as_double();
    ...
    double & yaw_error)
  {
    ...
    try {
      if (final_pose_map.header.frame_id != map_frame) {
        final_pose_map =
          tf_buffer_.transform(final_pose_map, map_frame, tf2::durationFromSec(0.05));
      }
    } catch (const tf2::TransformException & ex) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 2000,
  ...
  std::vector<std::string> getBaseFrameCandidates() const
  {
    ...
    const auto add_candidate = [&candidates](const std::string & frame) {
        if (!frame.empty() &&
          std::find(candidates.begin(), candidates.end(), frame) == candidates.end())
        {
